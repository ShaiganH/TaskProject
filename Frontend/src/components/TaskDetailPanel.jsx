...
import {
  X, Send, Pencil, Trash2, Clock, User, Tag, Calendar,
  CheckCircle2, XCircle, AlertCircle, Loader2, MessageSquare,
  Activity, ChevronDown, Circle, PauseCircle, Lock
} from "lucide-react";
...
import {
  formatDate, formatDateTime, getPriorityClass,
  statusLabel, actionLabel, actionClass,
} from "../utils/helpers";
import {
  getTaskById,
...
// ── Status config ─────────────────────────────────────────────────────────────
const STATUS_CFG = {
  Todo:       { label: "To Do",       icon: Circle,       color: "#6b7280", barColor: "#9ca3af", step: 0 },
  InProgress: { label: "In Progress", icon: Loader2,      color: "#2563eb", barColor: "#3b82f6", step: 1 },
  OnHold:     { label: "On Hold",     icon: PauseCircle,  color: "#d97706", barColor: "#f59e0b", step: 1 },
  Completed:  { label: "Completed",   icon: CheckCircle2, color: "#16a34a", barColor: "#22c55e", step: 2 },
  Cancelled:  { label: "Cancelled",   icon: XCircle,      color: "#dc2626", barColor: "#ef4444", step: 2 },
};

const PRIORITY_CFG = {
  Low:      { bg: "bg-green-50",  text: "text-green-700",  ring: "ring-green-200",  dot: "bg-green-400"  },
  Medium:   { bg: "bg-yellow-50", text: "text-yellow-700", ring: "ring-yellow-200", dot: "bg-yellow-400" },
...
function StatusProgressBar({ status }) {
  ...
  const widthMap = { Todo: "0%", InProgress: "50%", OnHold: "50%", Completed: "100%", Cancelled: "100%" };
  ...
  return (
    <div className="px-5 pb-5 pt-2">
      ...
      <div className="flex justify-between mb-2">
        {steps.map((s, i) => (
          <span key={i} className={`text-[10px] font-semibold transition-colors duration-300 ${s.active ? "text-gray-500" : "text-gray-300"}`}>
            {s.label}
          </span>
        ))}
      </div>
      ...
      <div className="flex justify-between mt-2">
        {[0, 1, 2].map((i) => {
          const stepIndex = isCancelled || isCompleted ? 2 : isInProgress || isOnHold ? 1 : 0;
          ...
          return (
            <motion.div
              ...
              style={{
                background: current ? cfg.barColor : active ? "#9ca3af" : "#e5e7eb",
      ...
      {/* Status pill */}
      <motion.div key={status} initial={{ opacity: 0, y: 3 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="flex justify-center mt-3">
        <span
          className="inline-flex items-center gap-1.5 text-[10px] font-bold tracking-widest uppercase px-3 py-1 rounded-full"
          style={{ background: `${cfg.barColor}15`, color: cfg.barColor, border: `1px solid ${cfg.barColor}30` }}
        >
          {(() => { const Icon = cfg.icon; return <Icon size={10} className={isInProgress ? "animate-spin" : ""} style={{ animationDuration: "2.5s" }} />; })()}
          {cfg.label}
        </span>
      </motion.div>
    </div>
...
function StatusDropdown({ value, onChange, disabled }) {
  ...
  return (
    <div className="relative" ref={ref}>
      ...
      >
        <Icon size={12} style={{ color: cfg.color, animationDuration: "2.5s" }} className={value === "InProgress" ? "animate-spin" : ""} />
          ...
          >
            {Object.entries(STATUS_CFG).map(([key, sc]) => {
              ...
              return (
                ...
                >
                  <SIcon size={13} style={{ color: sc.color }} className={key === "InProgress" ? "animate-spin" : ""} />
                  <span style={{ color: isSel ? sc.color : "#374151" }}>{sc.label}</span>
                  {isSel && (
                    <motion.div layoutId="ddot" className="ml-auto w-1.5 h-1.5 rounded-full" style={{ background: sc.color }} />
                  )}
                </motion.button>
              );
            })}
...
export default function TaskDetailPanel({ task, onClose, onRefresh }) {
  ...
  useEffect(() => {
    ...
    const onComment = (p) => {
      ...
      setDetail((prev) => {
        ...
        return { ...prev, comments: [...(prev.comments ?? []), { id: p.id, content: p.content, userId: p.userId, createdAt: p.createdAt }] };
      });
    };
    const onStatus  = (p) => { if (p.id === task.id) setDetail((d) => d ? { ...d, status: p.status } : d); };
    const onUpdated = (p) => { if (p.id === task.id) setDetail((d) => d ? { ...d, ...p } : d); };
  ...
  const isLocked    = display.status === "Completed" || display.status === "Cancelled";
  const isOverdue   = display.dueDate && new Date(display.dueDate) < new Date() && !isLocked;

  const priorityCfg = PRIORITY_CFG[display.priority] ?? PRIORITY_CFG.Medium;

  const handleStatusChange = (s) => {
    if (s === "Completed" || s === "Cancelled") { setPendingStatus(s); setStatusConfirm(true); return; }
    applyStatusChange(s);
  };
  ...
  const applyStatusChange = async (newStatus) => {
    ...
    try { await updateStatusAPI(display.id, newStatus); toast.success("Status updated"); onRefresh?.(); }
    catch (err) { setDetail((d) => d ? { ...d, status: prev } : d); toast.error(err?.response?.data.error ?? err?.response?.data ?? "Failed");}
  };
  ...
  const handleSendComment = async () => {
    ...
    setDetail((p) => p ? { ...p, comments: [...(p.comments ?? []), { id: oid, content: text, userId: user.id, createdAt: new Date().toISOString() }] } : p);
    setTimeout(() => commentEndRef.current?.scrollIntoView({ behavior: "smooth" }), 60);
    try { await postCommentAPI(display.id, text); await fetchDetail(); toast.success("Comment added"); }
    catch (err) {
      setDetail((p) => p ? { ...p, comments: (p.comments ?? []).filter((c) => c.id !== oid) } : p);
      setComment(text); toast.error(err?.response?.data.error ?? err?.response?.data ?? "Failed to add comment");
  ...
  const handleSaveEdit = async () => {
    setSaving(true);
    try {
      await updateTaskAPI(display.id, { title: form.title, description: form.description, priority: form.priority, assignedToUserId: display.assignedToUserId, dueDate: form.dueDate || null, categoryId: form.categoryId || null });
      await fetchDetail(); setEditing(false); toast.success("Task updated"); onRefresh?.();
    } catch (err) { toast.error(err?.response?.data?.error ?? err?.response?.data ?? "Failed"); }
    finally { setSaving(false); }
  };

  const handleDelete = async () => {
    try { await deleteTaskAPI(display.id); toast.success("Task deleted"); onClose(); onRefresh?.(); }
    catch (err) { toast.error(err?.response?.data?.error ?? err?.response?.data ?? "Failed"); }
  };
...
>
        ...
        <div
          ...
          style={{ background: "linear-gradient(160deg, #f8fafc 0%, #f1f5f9 100%)", borderBottom: "1px solid #e5e7eb" }}
        >
          ...
          <div className="relative px-5 pt-4">
                ...
                {canFullEdit && !isLocked && (
                  <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.94 }}
                    ...
                    className={`flex items-center gap-1.5 text-xs font-medium px-2.5 py-1.5 rounded-lg border transition-all ${
                      editing ? "bg-brand-50 text-brand-700 border-brand-200" : "bg-white text-gray-500 border-gray-200 hover:border-gray-300 hover:text-gray-700 shadow-sm"
                    }`}
                  >
            ...
            <AnimatePresence mode="wait">
              {loading ? (
                <motion.div key="skeleton" className="space-y-2 pb-5 animate-pulse">
                ...
                <motion.div key="edit" initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="space-y-3 pb-5">
                  <input
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <input type="date" className="border border-gray-200 bg-white rounded-xl px-3 py-2 text-xs text-gray-600 focus:outline-none focus:ring-2 focus:ring-brand-300 transition-all shadow-sm" value={form.dueDate} onChange={(e) => setForm((f) => ({ ...f, dueDate: e.target.value }))} />
                    <select className="border border-gray-200 bg-white rounded-xl px-3 py-2 text-xs text-gray-600 focus:outline-none focus:ring-2 focus:ring-brand-300 transition-all shadow-sm" value={form.categoryId} onChange={(e) => setForm((f) => ({ ...f, categoryId: e.target.value }))}>
                  ...
                  <div className="flex gap-2">
                    <motion.button whileTap={{ scale: 0.97 }} onClick={handleSaveEdit} disabled={saving}
                ...
                <motion.div key="view" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="pb-4">
                  {/* Chips */}
                  <div className="flex flex-wrap gap-1.5 mb-3">
                    <span className={`inline-flex items-center gap-1 text-[10px] font-semibold px-2 py-0.5 rounded-full ring-1 ${priorityCfg.bg} ${priorityCfg.text} ${priorityCfg.ring}`}>
                      <span className={`w-1.5 h-1.5 rounded-full ${priorityCfg.dot}`} />
                      {display.priority}
                    </span>
                    {cat && (
                      <span className="inline-flex items-center gap-1 text-[10px] font-semibold px-2 py-0.5 rounded-full ring-1"
                        style={{ background: `${cat.color}15`, color: cat.color, boxShadow: `0 0 0 1px ${cat.color}25` }}
                      >
                        <Tag size={8} />{cat.name}
                      </span>
                    )}
                  ...
                  {/* Title */}
                  <h2 className="text-[15px] font-bold text-gray-900 leading-snug">{display.title}</h2>
                </motion.div>
              )}
            </AnimatePresence>

            {/* Progress bar — flush to header bottom */}
            {!loading && !editing && <StatusProgressBar status={display.status} />}
          </div>
        </div>
              ...
              <>
                ...
                {display.description && (
                  ...
                  >
                    ...
                    <p className="text-xs text-gray-600 leading-relaxed">{display.description}</p>
                  </motion.div>
                )}
                  ...
                  {[
                    {
                      ...
                      value: <StatusDropdown value={display.status} onChange={handleStatusChange} disabled={!canStatus || isLocked} />
                    },
                    {
                      icon: User, label: "Assigned to",
                      value: assignee ? (
                        <button
                    ...
                    },
                    {
                      ...
                      value: <span className={`text-xs font-medium ${isOverdue ? "text-red-500" : "text-gray-700"}`}>{formatDate(display.dueDate) ?? <span className="text-gray-400 font-normal">Not set</span>}</span>
                    },
                    {
                      icon: Clock, label: "Created",
                      value: <span className="text-xs text-gray-500">{formatDate(display.createdAt)}</span>
                    },
                ...
                <motion.div initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
                  ...
                  <div className="flex gap-1 p-1 bg-gray-100 rounded-xl mb-4">
                    {[
                      { id: "comments", label: "Comments", icon: MessageSquare, count: taskComments.length },
                      { id: "activity", label: "Activity",  icon: Activity,       count: taskAudit.length  },
                    ].map((tab) => (
                      <motion.button
                        key={tab.id}
                      ...
                      >
                        ...
                        <span className="relative flex items-center gap-1.5">
                          <tab.icon size={11} />
                          {tab.label}
                          <span className={`text-[9px] font-bold px-1.5 py-0.5 rounded-full ${activeTab === tab.id ? "bg-gray-100 text-gray-500" : "bg-gray-200/60 text-gray-400"}`}>
                            {tab.count}
                          </span>
                        </span>
                      </motion.button>
                  ...
                  <AnimatePresence mode="wait">
                    {activeTab === "comments" ? (
                        ...
                        <div className="space-y-3 mb-4 min-h-[60px]">
                          {taskComments.length === 0 ? (
                            <div className="flex flex-col items-center py-8 text-gray-300">
                              <MessageSquare size={26} strokeWidth={1.3} className="mb-2" />
                              <p className="text-xs text-gray-400 font-medium">No comments yet</p>
                              <p className="text-[10px] text-gray-300 mt-0.5">Start the conversation</p>
                            </div>
                          ) : taskComments.map((c, idx) => {
                            const u = allUsers.find((u) => u.id === c.userId);
                            const initials = ((u?.firstName?.[0] ?? "") + (u?.lastName?.[0] ?? "")) || "?";
                            const name = u ? `${u.firstName} ${u.lastName}` : (c.userName ?? "Unknown");
                            ...
                            return (
                              <motion.div
                                ...
                                animate={{ opacity: isOpt ? 0.65 : 1, y: 0 }}
                                transition={{ delay: idx * 0.03 }}
                                className="flex gap-2.5"
                              >
                                <div
                                  className="w-6 h-6 rounded-full flex-shrink-0 flex items-center justify-center text-[9px] font-bold mt-0.5 ring-1"
                                  style={{
                                    background: isMe ? "#eff6ff" : "#f5f3ff",
                                    color: isMe ? "#2563eb" : "#7c3aed",
                                    boxShadow: `0 0 0 1px ${isMe ? "#bfdbfe" : "#ddd6fe"}`,
                                  }}
                                >
                                  {initials}
                                </div>
                                <div className="flex-1 min-w-0">
                                  <div className="flex items-baseline gap-2 mb-1">
                                    ...
                                    {isOpt && <span className="text-[10px] text-blue-400 flex items-center gap-0.5"><Loader2 size={9} className="animate-spin" />Sending</span>}
                                  </div>
                      ...
                      >
                        {taskAudit.length === 0 ? (
                          <div className="flex flex-col items-center py-8 text-gray-300">
                            <Activity size={26} strokeWidth={1.3} className="mb-2" />
                          ...
                          <div className="relative space-y-0 pl-1">
                            ...
                            {taskAudit.map((a, idx) => {
                              ...
                              const name = u ? `${u.firstName} ${u.lastName}` : (a.userName ?? "Unknown");
                              return (
                                <motion.div
                                  key={a.id}
                                ...
                                >
                                  ...
                                  <div>
                                    <div className="flex items-center gap-1.5 flex-wrap mb-0.5">
                                      <span className={`text-[9px] font-bold px-1.5 py-0.5 rounded-md ${actionClass(a.action)}`}>
                                        {actionLabel(a.action)}
                                      </span>
                                      {a.oldValue && a.newValue && (
                                        <span className="text-[10px] text-gray-400">
                                          {a.oldValue}<span className="mx-1 text-gray-300">→</span><span className="text-gray-600 font-medium">{a.newValue}</span>
                                        </span>
                                      )}
                                    </div>
                                    <p className="text-[10px] text-gray-400">{name} · {formatDateTime(a.timestamp)}</p>
                                  </div>
                                </motion.div>
                              );
                            })}
      ...
      <ConfirmModal
        open={statusConfirm}
        onClose={() => { setStatusConfirm(false); setPendingStatus(null); }}
        onConfirm={() => { setStatusConfirm(false); applyStatusChange(pendingStatus); }}
